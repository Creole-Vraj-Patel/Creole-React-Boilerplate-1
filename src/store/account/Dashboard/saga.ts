...
import { deletePost, fetchAllPosts } from '../../../services/account/Dashboard/index';
import { GetFnResult } from '../../../interfaces';
...
export function* getAllPostsDetailsRequestEffect() {
  yield takeEvery(ActionEnums.GET_ALL_POSTS, function* () {
    ...
    try {
      const response: GetFnResult<typeof fetchAllPosts> = yield call(fetchAllPosts);
      if (response.data) {
        yield put({
...
export function* deletePostRequestEffect() {
  yield takeEvery(
    ...
    function* ({ payload }: { type: string; payload: string }) {
      ...
      try {
        const response: GetFnResult<typeof deletePost> = yield call(deletePost, payload);
...
export default function* DashboardSaga() {
  yield all([fork(getAllPostsDetailsRequestEffect), fork(deletePostRequestEffect)]);
}
